...
1. **Persistent job store (Postgres)** — заменить file-based jobs storage на БД-слой для multi-instance и recovery после рестарта.
...
5. **Docker/Deploy** — выделить отдельный worker-процесс для batch/training задач + health/readiness probes (без немедленного перехода в Kubernetes).
6. **Backtesting → product loop** — связать результаты backtest с UX выбора модели и автоподсказками по retrain.
7. **Расширение mypy coverage** — постепенно подключить `app/models` и `app/explainability` после локального рефакторинга.
